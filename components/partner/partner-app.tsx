...
export function PartnerApp({ initial }: { initial: PartnerAuthResult }) {
  ...
  if (status === "wrong_role") {
    return (
      <main className="prx-app flex min-h-dvh items-center justify-center bg-white px-5">
        <div className="w-full max-w-md space-y-6">
          <PrxLogo variant="compact" title="PRX" className="h-7 w-auto text-ink" />
          <h1 className="font-display text-4xl font-semibold leading-none tracking-[-0.04em] text-ink">Acesso só para parceiros</h1>
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <a href={homeUrl} className="inline-flex min-h-12 items-center justify-center rounded-[3px] border border-line px-5 text-[15px] font-medium text-ink hover:border-ink">
              Ir para o app
            </a>
          </div>
        </div>
